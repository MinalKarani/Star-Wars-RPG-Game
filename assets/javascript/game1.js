$(document).ready(function () {
	...
	function startGame() {
		...
		$.each(characterSelection, function(index, char) {
            // create a div for each character to display character selection at start of the game
            console.log(char);
			var newCharacterDiv = $("<div>");
            newCharacterDiv.addClass("col-lg-2 characters");
            newCharacterDiv.attr("id",char.id);
			$("<div>").addClass("characters-heading").html(char.name).appendTo(newCharacterDiv);
			$("<div>").addClass("characters-body").append("<img src='" + char.img + "'>").appendTo(newCharacterDiv);
			$("<div>").addClass("characters-footer").append("<span class='hp'>" + char.healthPoints + "</span>").appendTo(newCharacterDiv);
			...
			if(character === null) {
				...
				$.each(characterSelection, function(index, character) {
					...
					if(character.id !== charId) {
						...
						$("#"+character.id).removeClass("characters").addClass("defenders").appendTo("#defenderArea");
					} else {
